...
**IMPORTANTE** existe uma demo publicada no heroku [clicar aqui](https://eng-eform-backend.herokuapp.com/), o primeiro acesso pode levar até 30s para ser estabelecido devido a conta ser gratuita.
...
> Isso permite adotar estratégias _SOLID-like_ entre outras que, não necessariamente precisam ser programadas.
...
**IMPORTANTE**: É possível conectar-se ao banco de produção usado no ambiente do Heroku apartir de qualquer client de conexão de PostgreSQL.
...
| PORT     | 5432                                                             |
| USER     | ggyxusavgpbpvz                                                   |
| PASSWORD | 2e94a7c80eca90815b1acdcb06a702b6c4d3b8ba6280cb9b2123bc76f23635c5 |
| DBNAME   | d61i6jp9grpedk                                                   |

**OBS** Para mais informações veja o valor presente na variável [DATABASE_URL](.env.example)
...
- `#GET /` - Rota não autenticada, devolve a lista de formulários para preenchimento.
- `#GET /{id}` - Rota não autenticada, devolve um formulário pelo id.
...
**IMPORTANTE** A API answers reserá revisada em breve, provavelmente deixará de fazer parte da estrutura de APIs publicas

- `#GET /v1/answers` - Rota autenticada, devolve uma lista de respostas paginados.
- `#POST /v1/answers` - Rota autenticada, registra novas respostas.
- `#GET /v1/answers/{id}` - Rota autenticada, devolve uma resposta pelo id.
- `#PATCH /v1/answers/{id}` - Rota autenticada, atualiza uma resposta pelo id.
- `#DELETE /v1/answers/{id}` - Rota autenticada, remove uma resposta pelo id.

### **Swagger:** <a name="swagger"></a>

**Note:** Toda a API encontrasse documentada via [Swagger](https://swagger.io/), para acessa-la basta usar o link abaixo:

[https://eng-eform-backend.herokuapp.com/api](https://eng-eform-backend.herokuapp.com/api)
