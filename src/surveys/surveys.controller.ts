...
export class SurveysController {
  ...
  async getAll(
    ...
    try {
      ...
      return new SurveyPaginateResponseDto(form, null, surveys, take, skip, count);
    } catch (error) {
  ...
  async getChildAll(
    ...
    try {
      ...
      const parent = await this.surveyService.getSurvey({ id: surveyId });

      const { surveys, count } = await this.surveyService.getAll(options);

      return new SurveyPaginateResponseDto(form, parent, surveys, take, skip, count);
    } catch (error) {
      throw new BadRequestException();
    }
  }
